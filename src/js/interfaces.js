...
export class InputParser {
     ...
     * @returns {Array<{LayoutType: string, char: string, type: string}>} - 解析后的结果
     */
    parse(input) {
...
export class LayoutCalculator {
   ...
   * @returns {Array<{char: string, x: number, y: number, height: number, width: number, svgFile:string, }>} - 布局信息
   */
...
export class Renderer {
    /**
     * 渲染 SVG 元素
     * @param {Array<{char: string, x: number, y: number, height: number, width: number, svgFile:string,}>} layoutInfo - 布局信息
